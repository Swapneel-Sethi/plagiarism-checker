...
const DOCS = [
  { name: 'T1  Verbatin copy (Computation)', truth: { p: true, a: false, o: false },
  ...
  { name: 'T2  Original human narrative', truth: { p: false, a: false, o: true },
  ...
  { name: 'T3  LLM-tell AI text', truth: { p: false, a: true, o: false },
  ...
  { name: 'T4  Paraphrase (climate/carbon)', truth: { p: true, a: false, o: false },
  ...
  { name: 'T5  Uniform mechanical AI text', truth: { p: false, a: true, o: false },
  ...
  { name: 'T6  Mixed verbatim + original', truth: { p: true, a: false, o: false },
  ...
  { name: 'T7  Verbatim Gettysburg', truth: { p: true, a: false, o: false },
  ...
  { name: 'T8  Human blog voice', truth: { p: false, a: false, o: true },
  ...
  { name: 'T9  Very short text', truth: { p: false, a: false, o: true },
    text: 'I love my cat and her tiny paws.' },
  { name: 'T10 Empty / whitespace', truth: { p: false, a: false, o: true },
    text: '    ' }
];
...
(async () => {
  const dim = { p: 0, a: 0, o: 0 }; const total = DOCS.length; let all = 0;
  console.log('=== FULL E2E via live server (' + BASE + ') ===\n');
  console.log('TEST'.padEnd(40) + 'PLAG%'.padStart(7) + 'AI'.padStart(5) + '  verdict (P/A/O) vs truth');
  ...
  for (const d of DOCS) {
    ...
    const o = !plag && !ai;
    const got = (plag ? 'P' : '.') + (ai ? 'A' : '.') + (o ? 'O' : '.');
    const want = (d.truth.p ? 'P' : '.') + (d.truth.a ? 'A' : '.') + (d.truth.o ? 'O' : '.');
    if (plag === d.truth.p) dim.p++;
    if (ai === d.truth.a) dim.a++;
    if (o === d.truth.o) dim.o++;
    if (plag === d.truth.p && ai === d.truth.a && o === d.truth.o) all++;
  ...
  console.log('  Originality         : ' + dim.o + '/' + total + '  = ' + acc(dim.o));
  console.log('  All-three correct   : ' + all + '/' + total + '  = ' + acc(all));
})();
